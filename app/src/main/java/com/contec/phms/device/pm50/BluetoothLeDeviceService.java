...
public class BluetoothLeDeviceService extends com.contec.phms.device.template.BluetoothLeDeviceService {
    private final String TAG = "BluetoothLeDeviceService_pm50";
        ...
        if (this.arrangeMessage != 0 && (this.arrangeMessage & 255) != 136) {
            ...
            switch (this.arrangeMessage) {
                case Byte.MIN_VALUE:
                    ...
                    Log.i("BluetoothLeDeviceService_pm50", "0x80  mAllNoUploadDataNumber:" + this.mAllNoUploadDataNumber);
                ...
                case -127:
                    this.mAllBoolPreNumber = this.packManager.mAllCount;
                    this.mAllNoUploadDataNumber = this.packManager.mCount;
                    Log.i("BluetoothLeDeviceService_pm50", "mAllNoUploadDataNumber:" + this.mAllNoUploadDataNumber);
                ...
                case OrderList.DM_NEXT_DEVICE_OR_POLLING /*66*/:
                    ...
                    if (dataCount > 0) {
                        ...
                        Log.i("jxx", "ui_update_progress method，计算的当前进度条值：" + progres + " dataCount:" + dataCount + " count:" + this.count);
                    }
                ...
                case 67:
                    ...
                    CLog.e("BluetoothLeDeviceService_pm50", "------数据个数:" + size);
                    if (size > 0) {
                        for (int i = 0; i < size; i++) {
                            ...
                            CLog.e("BluetoothLeDeviceService_pm50", " for 中的 date:" + _dateRecord);
                            ...
                            if (provalue != null) {
                                ...
                                FileOperation.writeToSDCard(String.valueOf(getcureentbytetime()) + " 校正时间：" + _date, "BluetoothLeDeviceService_pm50");
                                CLog.e("BluetoothLeDeviceService_pm50", " for 时间不正确，校正后 date:" + _date);
                            } else {
                                CLog.i("BluetoothLeDeviceService_pm50", "时间正确，不进行校正操作");
                                FileOperation.writeToSDCard(String.valueOf(getcureentbytetime()) + "时间正确，未对时间进行校正操作", "BluetoothLeDeviceService_pm50");
                            }
                            ...
                            mDeviceData.setmDataType(DeviceNameUtils.PM50);
    ...
    private void ui_receiving(int i, int size) {
        CLog.i("BluetoothLeDeviceService_pm50", "call ui_receiving method " + i + " size:" + size);
        int progress = Math.round(((((float) (i + 1)) / ((float) size)) * 15.0f) + 35.0f);
        if (this.lastProgress != progress) {
            DeviceManager.m_DeviceBean.mProgress = progress;
            Log.i("jxx", "计算的当前进度条值：" + DeviceManager.m_DeviceBean.mProgress);
